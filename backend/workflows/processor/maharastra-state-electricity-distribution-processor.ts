import { ElectricityBillProcessor } from './processors.js';
import { By, IWebDriverCookie, WebDriver } from 'selenium-webdriver';
import { Logger } from '../../core/logger.js';
import fetch from 'node-fetch';
import { electricityParam } from '../../config.js';

const logger: Logger = new Logger('MaharashtraStateElectricityDistributionProcessor');

export class MaharashtraStateElectricityDistributionProcessor implements ElectricityBillProcessor {
    async process(billConsumerNumber: string, driver: WebDriver): Promise<{ billAmount: number; billDueDate: Date } | undefined> {
        try {
            ...
            driver.sleep(2000);
            let response = await fetch('https://wss.mahadiscom.in/wss/wss?uiActionName=RefreshCaptchaViewPay&IsAjax=true', {
                headers: {
                    accept: '*/*',
            ...
            let amount = await driver.findElement(By.xpath(electricityParam.MAHARASHTRA_STATE_ELECTRICITY_DISTRIBUTION_CO_LTD.bill_amount_xpath)).getText();
            await driver.sleep(2000);
            let date = await driver.findElement(By.xpath(electricityParam.MAHARASHTRA_STATE_ELECTRICITY_DISTRIBUTION_CO_LTD.due_date_xpath)).getText();
            await driver.sleep(2000);
            return {
