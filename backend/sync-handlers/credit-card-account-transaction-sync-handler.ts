...
/**
 * Sync handler for credit card accounts.
 */
export class CreditCardAccountTransactionSyncHandler implements ISyncHandler<Account> {
    /**
     * Syncs transactions for a single credit card account.
     * @param account The credit card account to sync.
     * @param deltaSync Whether to perform a delta sync.
     */
    private syncSingleCreditCardAccount(account: Account, deltaSync: boolean) {
    ...
    /**
     * Syncs transactions for credit card accounts.
    ...
    sync(accounts: Account[], deltaSync: boolean): void {
        (async () => {
            ...
            let creditCardAccountsToSync = accounts.filter(
                (account) => account.account_type === 'CREDIT_CARD' && account.bank
            );
            ...
            const syncPromises = creditCardAccountsToSync.map((account) =>
                this.syncSingleCreditCardAccount(account, deltaSync)
            );
