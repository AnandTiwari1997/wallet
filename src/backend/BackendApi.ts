...
export const getAllTransactions = async (
    ...
    let data = response.data.results.map<Transaction>((transaction) => {
        return {
            ...
            transaction_state: transaction.transaction_state.toString(),
...
export const getBills = async (requestBody: ApiRequestBody<any> = {}, cancel: AbortController): Promise<ApiResponse<Bill>> => {
    return await axios
...
export const syncStockAccount = async (
    requestBody: ApiRequestBody<DematAccount> = {}
): Promise<{
    message: string;
}> => {
    return await axios
        .post(`/wallet/stock/account/sync`, requestBody, {
            headers: {
                'Content-Type': 'application/json'
            }
        })
