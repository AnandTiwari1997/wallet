...
const DatePicker = ({
  ...
  const [month, setMonth] = useState(
    range?.startDate ? range.startDate.getMonth() : new Date().getMonth(),
  );
  const [year, setYear] = useState(
    range?.startDate ? range.startDate.getFullYear() : new Date().getMonth(),
  );
  const [selectionInPreview, setSelectionInPreview] = useState(false);
  const [selection, setSelection] = useState<
    | { start: string; middle: string[]; end: string; forward: boolean }
    | undefined
  >({
  ...
  const doesRenderSelectionSpan = (elementId: string) => {
    return (
      ...
      selection?.middle.includes(elementId) ||
  ...
  const renderDays = () => {
      ...
      while (
        ...
        previousMonthDates.push(
          <button key={formattedId} className={classNames}>
            <span className="render-day-number">
    ...
    while (
      isBefore(currentMonthDate, endOfCurrentMonth) ||
      isSameDay(currentMonthDate, endOfCurrentMonth)
    ) {
      ...
      currentMonthDates.push(
        ...
        >
          ...
          {doesRenderSoftSelectionSpan(formattedId) && (
            <span
              className={renderSoftSelectionSpanClasses(formattedId)}
            ></span>
          )}
          <span className={renderSelectedDaySpanClasses(formattedId)}>
    ...
    if (isNextMonth) {
      ...
      while (
        isBefore(nextMonthDate, nextMonthDateEnd) ||
        isSameDay(nextMonthDate, nextMonthDateEnd)
      ) {
  ...
  return (
    <div className="render-date-picker" id="date-picker">
      <div className="render-header">
        <i
          aria-hidden="true"
          className="calender-picker-icon icon arrow-container left"
        ...
        <span className="render-header-month-year">
          <span>
            <select
              value={month}
              onChange={(evt) => setMonth(parseInt(evt.currentTarget.value))}
            >
          ...
          <span>
            <select
              value={year}
              onChange={(evt) => setYear(parseInt(evt.currentTarget.value))}
            >
