  ...
  const renderDays = () => {
    ...
    for (let i = 1; i <= days[month]; i++) {
      ...
      if (
        ...
        cName += " selected-week";
        if (
          startOfDay(new Date(week.firstDay)).getTime() ===
          startOfDay(new Date(currentDate)).getTime()
        ) {
          cName += " selected-week-start";
        }
        if (
          endOfDay(new Date(currentDate)).getTime() ===
          endOfDay(new Date(week.lastDay)).getTime()
        ) {
